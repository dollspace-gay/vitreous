# Feature: Vitreous GUI Framework — Architecture Specification
...
| D-18 | Canvas API uses platform-specific paint signatures (promoted from OQ-6) | Maximum power: `Fn(&wgpu::Device, &Queue, &TextureView)` on desktop, `Fn(&CanvasRenderingContext2d)` on web. Canvas code is not cross-platform by design. |
...
No open questions remain. All decisions are resolved — see Resolved Decisions table.
